...
class BME680 {
  ...
  constructor(i2cPort, slaveAddress) {
    ...
    this._LOOKUP_TABLE_2 = [
      ...
      127110228.0, 64000000.0, 32258064.0, 16016016.0, 8000000.0, 4000000.0,
      2000000.0, 1000000.0, 500000.0, 250000.0, 125000.0,
    ];
  ...
  async setup() {
    ...
    this.refresh_rate = 10; // default refresh rate
    this._min_refresh_time = 1 / this.refresh_rate;
  }
  async _read_calibration() {
    ...
    var coeff = new Uint8Array([...coeff1, ...coeff2]);
    ...
    this._pressure_calibration.push(
      this.getSVal2((coeff[12] << 8) | coeff[11]),
    ); // 7
    this._pressure_calibration.push(
      this.getSVal2((coeff[14] << 8) | coeff[13]),
    ); // 8
    this._pressure_calibration.push(this.getSVal1(coeff[16])); // 10
    this._pressure_calibration.push(this.getSVal1(coeff[15])); // 9
    this._pressure_calibration.push(
      this.getSVal2((coeff[20] << 8) | coeff[19]),
    ); // 12
    this._pressure_calibration.push(
      this.getSVal2((coeff[22] << 8) | coeff[21]),
    ); // 13
  ...
  async _perform_reading() {
    ...
    await this.i2cSlave.write8(
      this._BME680_REG_CTRL_MEAS,
      (this._temp_oversample << 5) | (this._pressure_oversample << 2),
    );

    // turn on humidity oversample
    await this.i2cSlave.write8(
      this._BME680_REG_CTRL_HUM,
      this._humidity_oversample,
    );
  ...
  gas() {
    ...
    var var1 =
      ((1340 + 5 * this._sw_err) * this._LOOKUP_TABLE_1[this._gas_range]) /
      65536;
