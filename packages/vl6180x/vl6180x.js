...
class VL6180X {
	...
	async init() {
		...
		if ( await this._read8(this._VL6180X_REG_IDENTIFICATION_MODEL_ID) != 0xB4){
			console.error("Could not find VL6180X, is it connected and powered?");
			return;
		}

		console.log( "Model ID:",(await this._read8(this._VL6180X_REG_IDENTIFICATION_MODEL_ID) ).toString(16));
	...
	set offset(offset) {
		...
		this._write8(this._VL6180X_REG_SYSRANGE_PART_TO_PART_RANGE_OFFSET, offset & 0xFF);
		this._offset = offset;
	}
	...
	async start_range_continuous(period = 100) {
		if (period < 20 || period > 2550 || period % 10 !== 0) {
			throw new Error("Delay must be in 10 millisecond increments between 20 and 2550 milliseconds");
		}
		const periodReg = Math.floor(period / 10) - 1;
		await this._write8(this._VL6180X_REG_SYSRANGE_INTERMEASUREMENT_PERIOD, periodReg);
	...
	async _read_range_continuous() {
		...
		while (!((status >> 2) & 0x01)) {
			await sleep(10); // 短い待ち時間を挿入
			status = await this._read8(this._VL6180X_REG_RESULT_INTERRUPT_STATUS_GPIO);
		}
	...
	async ranges_from_history() {
		...
		const history = [];
		for (let i = 0; i < 16; i++) {
			history.push(await this._read8(this._VL6180X_REG_RESULT_HISTORY_BUFFER_0 + i));
		}
		return history;
	}
	...
	async range_history_enabled() {
		const historyCtrl = await this._read8(this._VL6180X_REG_SYSTEM_HISTORY_CTRL);
