...
class TCS34725 {
  ...
  async active(value) {
    ...
    this._active = value;
    var enable = await this.i2cSlave.read8(
      this._REGISTER_ENABLE | this._COMMAND_BIT,
    );
    if (value) {
      await this.i2cSlave.write8(
        this._REGISTER_ENABLE | this._COMMAND_BIT,
        enable | this._ENABLE_PON,
      );
      sleep(3);
      await this.i2cSlave.write8(
        this._REGISTER_ENABLE | this._COMMAND_BIT,
        enable | this._ENABLE_PON | this._ENABLE_AEN,
      );
    } else {
      await this.i2cSlave.write8(
        this._REGISTER_ENABLE | this._COMMAND_BIT,
  ...
  async sensor_id() {
    return await this.i2cSlave.read8(
      this._REGISTER_SENSORID | this._COMMAND_BIT,
    );
  }
  async integration_time(value) {
    if (!value) {
    ...
    this.i2cSlave.write8(
      this._REGISTER_ATIME | this._COMMAND_BIT,
      256 - cycles,
    );
  }
  async gain(value) {
    if (!value) {
      return this._GAINS[
        await this.i2cSlave.read8(this._REGISTER_CONTROL | this._COMMAND_BIT)
      ];
    }
    ...
    await this.i2cSlave.write8(
      this._REGISTER_CONTROL | this._COMMAND_BIT,
      index,
    );
  }
  async _valid() {
    var v = await this.i2cSlave.read8(
      this._REGISTER_STATUS | this._COMMAND_BIT,
    );
    return v & 0x01;
  }
  async read(t_lux) {
    ...
    var r = await this.i2cSlave.read16(
      this._REGISTER_RDATA | this._COMMAND_BIT,
    );
    var g = await this.i2cSlave.read16(
      this._REGISTER_GDATA | this._COMMAND_BIT,
    );
    var b = await this.i2cSlave.read16(
      this._REGISTER_BDATA | this._COMMAND_BIT,
    );
    var c = await this.i2cSlave.read16(
      this._REGISTER_CDATA | this._COMMAND_BIT,
    );
