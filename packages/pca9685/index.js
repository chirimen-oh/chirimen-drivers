...
class PCA9685 {
  ...
  init(minPulse, maxPulse, angleRange, noSetZero) {
        ...
        async (i2cSlave) => {
          ...
          if (!noSetZero) {
            for (var servoPort = 0; servoPort < 16; servoPort++) {
              this.setServo(servoPort, 0).then(
                /** @type {() => void} */ (/** @type {any} */ (resolve)),
                /** @type {() => void} */ (/** @type {any} */ (reject)),
              );
            }
          }
        },
  ...
  async setServo(servoPort, angle) {
