...
class ADS1x15 {
  ...
  constructor(i2cPort, slaveAddress) {
    ...
    this.i2cSlave = null;
    this.amplifierTable = [
      4.096, 4.096, 1.024, 1.024, 0.256, 0.256, 0.256, 0.256,
    ]; // 000,001,010,010,100,101,110,111 スペックシートと違う？
    this.amplifierConf = 1;
    this.prevCh = -1;
  }
  async init(isAds1115, amplifierSelection) {
    if (isAds1115) {
    ...
    if (
      amplifierSelection != undefined &&
      amplifierSelection >= 0 &&
      amplifierSelection < 8
    ) {
  ...
  async read(channel) {
    var config;
    if (channel == "0,1") {
      // Differential Mode p0,p1
      config = 0x0000;
    } else if (channel == "0,3") {
      // Differential Mode p0,p3
      config = 0x1000;
    } else if (channel == "1,3") {
      // Differential Mode p1,p3
      config = 0x2000;
    } else if (channel == "2,3") {
      // Differential Mode p2,p3
      config = 0x3000;
    } else {
  ...
  getVoltage(rawVal) {
    return (
      (this.amplifierTable[this.amplifierConf] *
        2 *
        this.getSignedVal(rawVal)) /
      (this.bitShift == 4 ? 0x0fff : 0xffff)
    );
  }
}
