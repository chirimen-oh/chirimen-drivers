...
class CCS811 {
  ...
  async init(mode) {
    ...
    if (
      mode != this.CCS811_DRIVE_MODE_IDLE &&
      mode != this.CCS811_DRIVE_MODE_1SEC &&
      mode != this.CCS811_DRIVE_MODE_10SEC &&
      mode != this.CCS811_DRIVE_MODE_60SEC &&
      mode != this.CCS811_DRIVE_MODE_250MS
    ) {
  ...
  async checkError() {
    ...
    if ((err & 0b00000001) == 1) {
      ...
      if ((err & 0b10000000) > 0 && (err & 0b00010000) > 0) {
        console.log(
          "OK! : Valid application firmware loaded and Firmware is in application mode.",
        );
        return true;
      } else {
