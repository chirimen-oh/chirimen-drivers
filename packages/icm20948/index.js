...
class ICM20948 {
  ...
  async WriteSecondary(u8I2CAddr, u8RegAddr, u8data) {
    var u8Temp = 0;
    await this._write_byte(this.REG_ADD_REG_BANK_SEL, this.REG_VAL_REG_BANK_3); //swtich bank3
  ...
  async _read_byte(cmd) {
    return await this.i2cSlave.read8(cmd);
  }
  async _read_block(reg, length) {
  ...
  async _write_byte(cmd, val) {
    await this.i2cSlave.write8(cmd, val);
  }

  imuAHRSupdata(gx, gy, gz, ax, ay, az, mx, my, mz) {
    var norm = 0.0;
    var hx = 0,
      hy = 0,
      hz = 0,
