...
class NEOPIXEL_I2C {
  ...
  async setPixel(pixelNumber, red, green, blue) {
    if (this.GLOBALMODE) {
    ...
    await this.i2cSlave.write8(
      pixelNumber * 3 + this.pixRegStart,
      green & 0xff,
    );
    await this.i2cSlave.write8(
      pixelNumber * 3 + 1 + this.pixRegStart,
      red & 0xff,
    );
    await this.i2cSlave.write8(
      pixelNumber * 3 + 2 + this.pixRegStart,
      blue & 0xff,
    );
  }
  async setPixels(rgbArray, startPixel) {
    if (rgbArray.length / 3 + startPixel > this.N_LEDS) {
      throw Error(
        "pixelNumber overflow : rgbArray.length , startPixel , this.N_LEDS: " +
          rgbArray.length +
          "," +
          startPixel +
          "," +
    ...
    for (var i = 0; i < rgbArray.length; i++) {
      ...
      if (startAddr + i == 254) {
        // レジスタとして見えるピクセルの末尾でいったん書き出し(polifill srvのuint8バグ回避
  ...
  async setGlobal(red, green, blue) {
