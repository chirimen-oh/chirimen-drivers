...
class APDS9960 {
  ...
  async init() {
    ...
    if (devIdInvalid) {
      ...
      throw new Error("ADPS9960InvalidDevId: " + this.dev_id);
    }
  ...
  async enableLightSensor(interrupts) {
  ...
  async enableGestureSensor(interrupts) {
    if (interrupts == undefined) {
      interrupts = true;
    }
  ...
  async getProximityGain() {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_CONTROL);
    // shift and mask out PDRIVE bits
    return (val >> 2) & 0b00000011;
  }
  async setProximityGain(drive) {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_CONTROL);
    // set bits in register to given value
    drive &= 0b00000011;
    drive = drive << 2;
  ...
  async getAmbientLightGain() {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_CONTROL);
    // shift and mask out ADRIVE bits
    return val & 0b00000011;
  }
  async setAmbientLightGain(drive) {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_CONTROL);
    // set bits in register to given value
    drive &= 0b00000011;
    val &= 0b11111100;
  ...
  async getProxPhotoMask() {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_CONFIG3);
    // mask out photodiode enable mask bits
    return val & 0b00001111;
  }
  async setProxPhotoMask(mask) {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_CONFIG3);
    // set bits in register to given value
    mask &= 0b00001111;
    val &= 0b11110000;
  ...
  async getGestureLEDDrive() {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_GCONF2);
    // shift and mask out LED drive bits
    return (val >> 3) & 0b00000011;
  }
  async setGestureLEDDrive(drive) {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_GCONF2);
    // set bits in register to given value
    drive &= 0b00000011;
    drive = drive << 3;
  ...
  async getGestureWaitTime() {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_GCONF2);
    // shift and mask out LED drive bits
    return val & 0b00000111;
  }
  async setGestureWaitTime(time) {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_GCONF2);
    // set bits in register to given value
    time &= 0b00000111;
    val &= 0b11111000;
  ...
  async getProximityIntLowThreshold() {
    return await this.i2cSlave.read8(this.APDS9960_REG_PILT);
  }
  async setProximityIntLowThreshold(threshold) {
    await this.i2cSlave.write8(this.APDS9960_REG_PILT, threshold);
  }
  async getProximityIntHighThreshold() {
    return await this.i2cSlave.read8(this.APDS9960_REG_PIHT);
  }
  async setProximityIntHighThreshold(threshold) {
    await this.i2cSlave.write8(this.APDS9960_REG_PIHT, threshold);
  }
  async getAmbientLightIntEnable() {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_ENABLE);
    return ((val >> 4) & 0b00000001) == 1;
  }
  async setAmbientLightIntEnable(enable) {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_ENABLE);
  ...
  async getProximityIntEnable() {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_ENABLE);
    return ((val >> 5) & 0b00000001) == 1;
  }
  async setProximityIntEnable(enable) {
    var val = await this.i2cSlave.read8(this.APDS9960_REG_ENABLE);
    // set bits in register to given value
    val &= 0b11011111;
    if (enable) {
  ...
  async _read_i2c_block_data(cmd, num) {
  ...
  async _read_byte_data(, cmd) {
    return this.bus.read_byte_data(this.address, cmd)
  }
  async _write_byte_data(, cmd, val) {
