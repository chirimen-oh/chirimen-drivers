...
class INA219 {
  ...
  constructor(i2cPort, slaveAddress) {
    ...
    this.__AMP_ERR_MSG =
      "Expected current is greater than max possible current ";
    this.__RNG_ERR_MSG =
    ...
    this.__LOG_MSG_3 =
      "Current overflow detected - attempting to increase gain";
  ...
  async init(shunt_ohms, max_expected_amps) {
  ...
  async configure(voltage_range, gain, bus_adc, shunt_adc) {
  ...
  // internal funcs
  async _handle_current_overflow() {
  ...
  _determine_gain(max_expected_amps) {
  ...
  async _configure(voltage_range, gain, bus_adc, shunt_adc) {
    var configuration =
  ...
  async _calibrate(bus_volts_max, shunt_volts_max, max_expected_amps) {
  ...
  _determine_current_lsb(max_expected_amps, max_possible_amps) {
    var current_lsb;
    if (max_expected_amps) {
  ...
  async __read_register(register, negative_value_supported) {
